...
const GROUPS = [
  ...
  ["Lobby & race flow", [
    ...
    { m: "POST", p: "/color/:x", s: "Set a racer's color", d: "Hex (#ff8800 / ff8800) or a name (red, cyan, orange, pink, gold, lime, …). Applied now (car label, leaderboard, overlays) and remembered for future joins. Omit `color` to forget the saved one; `/color/reset` forgets all. Viewers can do the same from chat with the command in Settings (default `!color`).", params: [TARGET, P("color", "string", "hex or name", { in: "query", example: "#ff8800" })], res: { ok: true, affected: 1 }, err: "400 bad color" },
  ...
  ["Chaos", [
    ...
    { m: "POST", p: "/boost/:x/add", s: "Add boosts to a car's !boost pool", params: [P("x", "string | all", "id, login, or `all`", { in: "path", example: "all" }), P("n", "int", "how many to add", { in: "query", default: 1 })], res: { ok: true, affected: 7 } },
    ...
    { m: "POST", p: "/respawn/:x", s: "The game's stuck-car respawn", d: "Blink for 2 s, drop back onto the track a little ahead.", params: [P("x", "string | all", "id, login, or `all`", { in: "path", example: "all" })], res: { ok: true, affected: 7 } },
  ]],
  ["Camera", [
    { m: "GET", p: "/camera", s: "Current camera state", params: [], res: { auto: true, mode: "pack", target: "4 cars", cars: ["filian", "faker", "snoopdogg", "elonmusk"], fov: 75 }, note: "mode: follow · free · chase · front · pack · sweep · side · high · orbit · overhead · prop · finish · manual" },
    { m: "POST", p: "/camera/auto", s: "Toggle the auto director", d: "Wide shots first (pack, side, sweep, high), the leader every other cut, the game's fixed track cams when a group approaches one, cuts to booms, a finish camera when cars near the line. Any camera key or mouse look by the streamer pauses it for 5 s.", params: [P("on", "0|1", "set instead of toggle", { in: "query" })], res: { auto: true, mode: "pack", target: "4 cars", cars: ["…"], fov: 75 } },
...
const EVENTS = [
  ...
  ["boom", "a car was boomed (one vehicle) – bits, API, anything"], ["boost", "a car used a pool boost (one vehicle)"], ["respawn", "a car was respawned via the API (one vehicle)"],
  ["developer", "a joined racer's backend title says Developer (one vehicle)"], ["camera", "{auto, mode, target, cars, fov} on every cut"],
  ["settings", "full settings after any PUT"], ["respawn", "a car was respawned (API or the chat command)"], ["color", "{login, color} when a racer's color is set (API or chat)"],
  ["perk", "{login, displayName, extraBoosts, reasons:[sub|dev|host|follower], boosts} — extra boosts granted on join (Settings → Perks)"],
  ["denied", "{login, command, tier} — a chat command was refused by the tier rule"],
    ...
    <div class="api">
      ...
      ${tab === "client" ? html`
        <article>
          ...
          <p class="desc">Browser sources: <code>/overlay</code> (track bar + leaderboard + effects) and <code>/minimap</code> (route + dots) are served by the plugin and reconnect on their own.</p>
        </article>` : null}
    </div>`;
}
