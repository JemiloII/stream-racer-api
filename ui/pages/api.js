...
const GROUPS = [
  ["Race state", [
    ...
    { m: "GET", p: "/twitch/auth", s: "Log the mod into Twitch (opens the Twitch consent page)", d: "Implicit OAuth with your own Twitch app (`settings.twitchClientId`; add the redirect URI to the app first). Asks for moderator:read:followers, user:write:chat, user:read:chat, channel:read:subscriptions. The token lands in settings; follower checks and chat replies use it.", params: [P("clientId", "string", "override/set the client id", { in: "query" })], res: { redirect: "302 → id.twitch.tv" }, err: "400 no client id" },
    { m: "GET", p: "/twitch/token", s: "Twitch login status", params: [], res: { connected: true, login: "shibikox", userId: "23256990", clientId: "…", scopes: ["moderator:read:followers", "user:write:chat"], wanted: ["…"], redirectUri: "http://localhost:8793/twitch/callback", features: { followerChecks: true, chatReplies: true }, missing: [] } },
    { m: "POST", p: "/twitch/token", s: "Store a token (the callback page does this)", d: "Validated against id.twitch.tv; login, user id and scopes are remembered.", params: [P("body", "object", "{token, state?}", { in: "body" })], body: { token: "abc…" }, res: { connected: true, login: "shibikox" }, err: "401 token rejected" },
    { m: "DELETE", p: "/twitch/token", s: "Forget the Twitch login", params: [], res: { ok: true, affected: 1 } },
