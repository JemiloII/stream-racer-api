...
export default function ChatCommandsCard() {
  ...
  const saveRespawnLimit = (event) => +event.target.value !== (settings.respawnLimit ?? DEFAULT_RESPAWN_LIMIT) && saveSettings({ respawnLimit: Math.max(0, +event.target.value || 0) });
  return html`
    <article>
      ...
      <div class="ovgrid">
        ...
        <label>Respawns per racer per race (0 = unlimited)<input type="number" min="0" step="1" key=${settings.respawnLimit ?? ""} defaultValue=${settings.respawnLimit ?? DEFAULT_RESPAWN_LIMIT} onBlur=${saveRespawnLimit} /></label>
