...
test.describe("Mini map browser source", () => {
  ...
  for (const aspect of ["16:9", "9:16", "4:3"]) {
    test(`?aspect=${aspect} draws a box of that ratio`, async ({ page }) => {
      await page.goto(`/minimap?aspect=${encodeURIComponent(aspect)}&alpha=0.6&mapTitle=0`);   // the title paints above the box and would widen the measured bounds // the box is transparent by default; give it a colour so its ratio can be read off the pixels
  ...
  test("without ?aspect the box follows settings.minimap.aspect", async ({ page, server }) => {
    ...
    await page.goto("/minimap?alpha=0.6&mapTitle=0"); // transparent by default: give the box a colour so the pixels show its ratio
